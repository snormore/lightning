...
use lightning_interfaces::types::{
    ...
    NodeRegistryChangeExitingReason,
    NodeServed,
    Participation,
    ProofOfConsensus,
    ProofOfMisbehavior,
...
impl<B: Backend> StateExecutor<B> {
    ...
    fn stake(
        ...
        let had_sufficient_stake_and_participating = node_index.map_or(false, |index| {
            self.has_sufficient_stake_and_participating(&index)
        });
        ...
        // for this block.
        // TODO(snormore): Double check that this is right.
        if !had_sufficient_stake_and_participating
            && self.has_sufficient_stake_and_participating(&node_index)
        {
    ...
    fn unstake(
        ...
        let had_sufficient_stake = self.has_sufficient_stake(&node_index);
        ...
        // If the node no longer has sufficient stake but previously did, record it as
        // exiting in the node registry changes for this block.
        // TODO(snormore): Double check that this is right.
        if had_sufficient_stake && !self.has_sufficient_stake(&node_index) {
            self.record_node_registry_change(
                node_public_key,
                NodeRegistryChange::Exiting(NodeRegistryChangeExitingReason::Unstaked),
            );
        }
        ...
        measurements
            ...
            .for_each(|(peer_index, measurements)| {
                if peer_index != reporting_node
                    && self.has_sufficient_stake_and_participating(&peer_index)
                    && measurements.verify()
                {
    ...
    /// Whether a node has sufficient stake and is participating.
    fn has_sufficient_stake_and_participating(&self, node_index: &NodeIndex) -> bool {
        self.has_sufficient_stake(node_index) && self.is_participating(node_index)
    }
