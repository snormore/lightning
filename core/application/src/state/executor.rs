...
mod tests {
    ...
    use hp_fixed::unsigned::HpUfixed;
    use lightning_interfaces::types::{NodeIndex, NodeInfo, Participation, Staking};
    use rand::prelude::SliceRandom;

    // (dalton) Since the quick sort used to select the winners of the auctions takes &self of the
    // whole state, since it has to do reputation lookups on the compare nodes side of things I am
    // going to repeate the modified quick sort algorithm here so we can have unit tests on just the
    // algoritm
    #[test]
    fn test_quick_sort() {
        let nodes = quick_sort_mock_node_list();
        // We want the top 1000 nodes so the algorithm will find us and return the pivot from the
        // bottom 9000
        let k = 9000;
        let r = nodes.len() - 1;
    ...
    fn quick_sort_partition_repeated(
        nodes: &mut [(NodeIndex, NodeInfo)],
        l: usize,
        r: usize,
    ) -> usize {
        let pivot = nodes[r].clone();
