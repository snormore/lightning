...
use lightning_interfaces::types::{Epoch, EpochInfo, ExecuteTransactionRequest, UpdateMethod};
...
> {
    ...
    consensus_output_tx: mpsc::Sender<FilteredConsensusOutput>,
    /// Receive consensus output in the execution worker from the execution state.
    consensus_output_rx: Option<mpsc::Receiver<FilteredConsensusOutput>>,
    ...
    EpochState<Q, P, NE>
{
    pub fn new(
        ...
        pub_sub: P,
        consensus_output_tx: mpsc::Sender<FilteredConsensusOutput>,
        consensus_output_rx: mpsc::Receiver<FilteredConsensusOutput>,
    ...
    fn wait_to_signal_epoch_change(&self, mut time_until_change: Duration, epoch: Epoch) {
        ...
        tokio::task::spawn(async move {
            ...
            loop {
                let time_to_sleep = tokio::time::sleep(time_until_change);
                ...
                tokio::select! {
                    ...
                    _ = time_to_sleep => {
                        ...
                        tracing::info!("Narwhal: Signalling ready to change epoch");
                        ...
                        if let Err(e) = txn_socket
                            ...
                            .await
                        {
                            tracing::error!("Error sending change epoch signal to socket {}", e);
                        }
    ...
    async fn run_narwhal(
        ...
        tracing::info!("Node is on current committee, starting narwhal.");
                ...
                .and_then(|s| s.parse::<u64>().ok())
            {
                if epoch_num < current_epoch - retention {
                    if let Err(e) = fs::remove_dir_all(file.path()) {
                        tracing::error!("Unable to remove garbage collected Narwhal epoch: {e}");
                    }
                }
            }
        }
    } else {
        tracing::error!("Unable to read narwhal store directory to garbage collect old databases");
    }
}
