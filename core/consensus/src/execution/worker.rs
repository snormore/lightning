...
async fn handle_consensus_output<P: PubSub<PubSubMsg>, Q: SyncQueryRunnerInterface, NE: Emitter>(
    ...
    if response.change_epoch {
        ...
        // TODO(snormore): Emit notifier event to indicate committee and active node set changes, so
        // that other components like the pool and topology can react to changes.
    }
}
