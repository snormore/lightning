...
//     // TODO(snormore): Figure out why this test fails sometimes when using ics23 proofs in JMT,
// but     // not when using `jmt::SparseMerkleProof`.
...
//             // Generate proof of existence.
//             let proof = M::get_state_proof(ctx, "node",
// M::Serde::serialize(&node_index)).unwrap();

//             // Verify proof of existence.
//             proof
//                 .verify_membership::<NodeIndex, NodeInfo, M>("node", node_index, value,
// state_root)                 .unwrap();
//         });
//     }
// }
