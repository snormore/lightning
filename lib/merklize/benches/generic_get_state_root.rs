...
fn bench_generic_get_state_root_rocksdb_jmt_keccak256_simple(b: &mut Bencher) {
    ...
    generic_bench_get_state_root::<JmtStateTree<_, DefaultSerdeBackend, KeccakHasher>>(
        b,
...
fn bench_generic_get_state_root_rocksdb_jmt_blake3_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_get_state_root::<JmtStateTree<_, DefaultSerdeBackend, Blake3Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_get_state_root_rocksdb_jmt_sha256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_get_state_root::<JmtStateTree<_, DefaultSerdeBackend, Sha256Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_get_state_root_rocksdb_mpt_keccak256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_get_state_root::<MptStateTree<_, DefaultSerdeBackend, KeccakHasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_get_state_root_rocksdb_mpt_blake3_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_get_state_root::<MptStateTree<_, DefaultSerdeBackend, Blake3Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_get_state_root_rocksdb_mpt_sha256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_get_state_root::<MptStateTree<_, DefaultSerdeBackend, Sha256Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn generic_bench_get_state_root<T: StateTree>(
    b: &mut Bencher,
    ...
    b.iter(|| {
        db.query().run(|ctx| {
            <T::Reader as StateTreeReader<T>>::get_state_root(ctx).unwrap();
        });
    })
}
