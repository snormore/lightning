...
use crate::{MerklizedContext, MerklizedStrategy, SimpleHasher, StateKey};

pub(crate) const NODES_TABLE_NAME: &str = "%state_tree_nodes";
pub(crate) const KEYS_TABLE_NAME: &str = "%state_tree_keys";

/// A merklized strategy that uses a Jellyfish Merkle Tree (JMT) implementation ([`jmt`]) to manage
    ...
    for JmtMerklizedStrategy<B, S, H>
{
    ...
    /// Return the ICS23 proof spec for the merklized strategy, customized to the specific hasher.
