...
use merklized::{DefaultMerklizedStrategyWithHasherKeccak, MerklizedAtomoBuilder};
use rand::rngs::StdRng;
use rand::SeedableRng;
use tokio::sync::Mutex;

pub async fn execute_txn_and_query_complex<B: StorageBackend>(
    env_: Arc<Mutex<Env<UpdatePerm, B>>>,
    num_txns: usize,
) {
    let mut env = env_.lock().await;
    ...
    let _res = env.run(block, || DummyPutter {}).await;
...
pub async fn execute_txn_and_query_medium<B: StorageBackend>(
    env_: Arc<Mutex<Env<UpdatePerm, B>>>,
    num_txns: usize,
) {
    let mut env = env_.lock().await;
...
pub async fn execute_txn_and_query_simple<B: StorageBackend>(env_: Arc<Mutex<Env<UpdatePerm, B>>>) {
    let stake_amount = 1000_u64;
    let mut env = env_.lock().await;
    let secret_key = AccountOwnerSecretKey::generate();
    let public_key = secret_key.to_pk();
...
pub fn create_merklized_app_env<C: StorageBackendConstructor>(
    ...
    let db = MerklizedAtomoBuilder::<
        _,
        DefaultSerdeBackend,
        DefaultMerklizedStrategyWithHasherKeccak<C::Storage>,
    >::new(storage)
...
#[allow(unused)]
pub fn create_transfer_tx(token: Tokens, amount: u64, to: EthAddress) -> UpdateMethod {
    let amount: HpUfixed<18> = amount.into();
    UpdateMethod::Transfer { token, amount, to }
}
